...
void SetMainState(Te_MoudleId_u8 module_id, Te_ModuleMainState_u8 main_state)
{
  ...
  if (main_state == state.main_state)
    return;
  SSM_LOG(Te_AppLogLevel_INFO, "app_printf####module id %d request\n", module_id);
  SSM_LOG(Te_AppLogLevel_INFO, "app_printf####main state %d to %d\n", state.main_state, main_state);
  SSM_LOG(Te_AppLogLevel_INFO, "app_printf####sub  state %d\n", state.sub_state);
  if(0 != CheckRightful_StateChange(module_id, main_state))
    return;
  
  /**maybe state transfer path should support a context**/
  if (state.sub_state == Te_SubState_StopSucceed)
  {
    state.main_state = main_state;
  ...
  SSM_LOG(Te_AppLogLevel_INFO, "app_printf####module id %d result\n", module_id);
  SSM_LOG(Te_AppLogLevel_INFO, "app_printf####main state %d\n", state.main_state);
  SSM_LOG(Te_AppLogLevel_INFO, "app_printf####sub  state %d\n", state.sub_state);
}
...
int ModuleStateCheck_SSM(uint8_t this_module_id)
{
  ...
  SSM_LOG(Te_AppLogLevel_INFO, "ModuleStateCheck_SSM (module id:%d)\n", this_module_id);
  ...
  if (self_state.sub_state == Te_SubState_HeartBeatError)
  {
    ...
    SSM_LOG(Te_AppLogLevel_ERROR, "heart beat error\n");
    return -1;
  }
  ...
  {
    ReportModuleError_SSM(this_module_id, errorId);
    SSM_LOG(Te_AppLogLevel_ERROR, "module work stop failed\n");
  ...
  for (depend_module_id = 0; depend_module_id < Te_ModuleId_Count; depend_module_id++)
  {
    ...
    //SSM_LOG(Te_AppLogLevel_INFO, "app_printf####depend module id: %d; state: %d to %d\n", depend_module_id, depend_cur_mainstate, depend_dst_mainState);
    ...
    {      
      ...
      { 
        ReportModuleError_SSM(this_module_id, errorId);
        SSM_LOG(Te_AppLogLevel_ERROR, "self state is steady, depend module may be fail in work\n");
